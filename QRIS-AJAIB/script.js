...
window.buatPembayaran = async function () {
  ...
  const res = await fetch(`https://api.ramzzhost.my.id/orderkuota/createpayment?apikey=${apikey}&amount=${jumlahDeposit}&codeqr=${qrisCode}`);
...
async function cekStatusPembayaran() {
  ...
  try {
    const res = await fetch(`https://api.ramzzhost.my.id/orderkuota/mutasiqr?apikey=${apikey}&username=${username}&token=${mutasiqr}`);
    const json = await res.json();
